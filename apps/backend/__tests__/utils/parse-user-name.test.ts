...
describe("Parsing a User's Name for the database", () => {
	...
	describe("When given a name", () => {
		it("should return only first name value if last name not given", () => {
			const result = parseUserName("Izzy");
			expect(result?.firstName).toBe("Izzy");
			expect(result?.lastName).toBe("");
		});

		it("should return first and last name property", () => {
			const result = parseUserName("Izzy Vickers");
			expect(result?.firstName).toBe("Izzy");
			expect(result?.lastName).toBe("Vickers");
		});
		it("should return full name capitalized in either situation", () => {
		...
		it("should return entire last name if more than one, including hyphens", () => {
